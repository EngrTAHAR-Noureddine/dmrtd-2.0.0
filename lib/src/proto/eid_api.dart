...
class EidApi {
  ...
  EidApi(ComProvider com) : icc = ICC(com);
  ...
  /// Selects emirates ID application applet.
  /// Can throw [ICCError] if command is sent to invalid MRTD document.
  /// Can throw [ComProviderError] in case connection with MRTD is lost.
  Future<void> selectEidApplication() async {
    _log.debug("Selecting eID application");
    await icc.selectFileByDFName(dfName: EidDF.AID, p2: 0x00);
  }
  ...
  Future<Uint8List> readFile(final int fid) async {
    ...
    await icc.selectFileById(fileId: efId);
