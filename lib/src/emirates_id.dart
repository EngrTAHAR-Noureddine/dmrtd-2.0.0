...
class EmiratesId {
  ...
  Future<Uint8List> readRawDG11() async {
    _log.debug("Reading EF.DG11");
    await _selectDF1();
    return _exec(() => _api.readFileBySFI(EfDG11.SFI)
    );
  }
  ...
  Future<Uint8List> readRawDG12() async {
    _log.debug("Reading EF.DG12");
    await _selectDF1();
    return await _exec(() => _api.readFileBySFI(EfDG12.SFI)
    );
  }
  ...
  Future<Uint8List> readIdnCn() async {
    _log.debug("Reading IDN CN..");
    return await _exec(() => _api.readFile(0x0201)
    );
  }

  Future<Uint8List> readNonModData() async {
    _log.debug("Reading Non-Modifiable Data..");
    return await _exec(() => _api.readFile(0x0203)
    );
  }

  Future<Uint8List> readPhoto() async {
    _log.debug("Reading Photo..");
    return await _exec(() => _api.readFile(0x0202)
    );
  }

  Future<Uint8List> readModData() async {
    _log.debug("Reading Modifiable Data..");
    return await _exec(() => _api.readFile(0x0205)
    );
  }

  Future<Uint8List> readSignImage() async {
    _log.debug("Reading Signature Image..");
    return await _exec(() => _api.readFile(0x0207)
    );
  }
  ...
  Future<void> _selectDF1() async {
    if(_dfSelected != _DF.EID) {
      _log.debug("Selecting eID Applet..");
      await _exec(() =>
        _api.selectEidApplication()
      );
      _dfSelected = _DF.EID;
    }
  }
